...
# An agent envelope larger than this is summarised instead of shipped.
...
def _bounded_agents(state: dict[str, Any]) -> tuple[dict[str, Any], dict[str, int]]:
    """
    Agent envelopes for the wire, each capped independently.

    Serialising one agent at a time bounds the peak: a single oversized
    envelope is replaced by a summary rather than being held in memory
    alongside all the others. Returns (payload, byte sizes) — the sizes
    are reported to the client so an outsized agent is visible rather
    ...
    for node, envelope in _agents_of(state).items():
        try:
            cleaned = _clean_for_json(envelope)
            encoded = json.dumps(cleaned, ensure_ascii=False)
        except Exception as exc:  # noqa: BLE001
            payload[node] = {
                "status": _status_of_envelope(envelope) or "UNKNOWN",
                "error": f"could not be serialised: {exc}",
                "truncated": True,
            }
            sizes[node] = -1
            continue

        sizes[node] = len(encoded)

        if len(encoded) > MAX_AGENT_JSON_BYTES:
            payload[node] = {
                "status": _status_of_envelope(envelope) or "SUCCESS",
                "error": _error_of_envelope(envelope),
                "truncated": True,
                "bytes": len(encoded),
                "note": (
                    f"{node} returned {len(encoded)} bytes; trimmed to keep "
                    "the stream small."
                ),
            }
            continue

        payload[node] = cleaned
...
def _node_event(
    ...
    event: dict[str, Any] = {
        ...
        # Memory rides on node events too, not just heartbeats — while
        # nodes are completing there are no heartbeats, and that is
        # exactly the window where things go wrong.
        "memory_rss_mb": _rss_mb(),
    }
    ...
    if node == FINAL_NODE:
        ...
        # Carry the finished assessment on this event, not only on
        # `final`. The recommendation is small; the final event is
        # large. If the run dies between the two, the client still has
        ...
        event["timed_out"] = bool(plan.get("timed_out", False))
        event["rejection_reason"] = plan.get("rejection_reason")
        event["activity"] = plan.get("activity")
        event["date"] = plan.get("date")
...
def _final_event(
    ...
    plan = _plan_of(state)
    agents, agent_bytes = _bounded_agents(state)
    ...
    return {
        ...
        # frequently empty.
        "agents": agents,
        # Serialised size of each envelope before capping — makes an
        # outsized agent visible instead of merely suspected.
...
def _minimal_final_event(
    ...
    """
    The final event stripped to what the user came for.

    Used when the full one cannot be built or sent. The recommendation
    is a few kilobytes; the agent envelopes are what make the full
    event big, so dropping them is what makes this one survivable.
    """
...
@app.post("/api/ask")
def ask(payload: AskRequest, include_state: bool = False):
    """
    Run the full graph and return once the assessment is ready.

    Typically 15-40s. Prefer /api/ask/stream in a browser.

    The complete graph state is omitted unless ?include_state=1 — it
    contains every agent's raw output and can be far larger than the
    assessment itself.
    """
    ...
    agents, agent_bytes = _bounded_agents(final_state)
    ...
    if include_state:
        result["state"] = _clean_for_json(final_state)
    ...
    try:
        while True:
            ...
            if event_type == "graph_finished":
                duration = time.monotonic() - started
                _log(
                    f"graph finished in {duration:.1f}s rss={_rss_mb()}MB "
                    f"nodes={completed}"
                )

                # Building this frame is the single largest thing the
                # stream does, and it is the last thing that happens —
                # so it gets its own guard and its own fallback.
                try:
                    frame = _sse(
                        _final_event(payload, final_state, completed, duration)
                    )

                    if len(frame) > MAX_FINAL_EVENT_BYTES:
                        _log(
                            f"final event {len(frame)} bytes exceeds "
                            f"{MAX_FINAL_EVENT_BYTES}; sending the trimmed one"
                        )
                        frame = _sse(
                            _minimal_final_event(
                                payload,
                                final_state,
                                completed,
                                duration,
                                f"agent data was {len(frame)} bytes, over the "
                                f"{MAX_FINAL_EVENT_BYTES} byte stream limit",
                            )
                        )

                except Exception as exc:  # noqa: BLE001
                    traceback.print_exc()
                    _log(f"could not build the full final event: {exc!r}")
                    ...
                    frame = _sse(
                        _minimal_final_event(
                            ...
                            completed,
                            duration,
                            f"agent data could not be serialised: {exc}",
                        )
                    )

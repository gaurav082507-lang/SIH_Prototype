...
NON_COASTAL_REJECTION_REASON = "Given coordinates don't lie in a coastal region."

# Maximum distance (km) from a coastline for a point to still be
# considered "coastal" for this platform's purposes.
COASTAL_CHECK_MAX_RADIUS_KM = 100
...
Determine whether the user's question is marine-related AND whether the
given latitude/longitude are reasonably close to a coastline, sea, or ocean,
then select only the specialist agents actually needed.
...
Rules:
- Reject clearly non-marine questions.
- Reject if the given latitude/longitude are landlocked / not reasonably
  close to a coastline, sea, or ocean, even if the question itself sounds
  marine-related. Use your general geographic knowledge of coastlines to
  judge this. Example: New Delhi, India (~28.6 N, 77.2 E) is deep inland
  and must be rejected on this basis.
- When rejecting because the coordinates are not coastal, set
  rejection_reason to exactly: "{non_coastal_reason}"
...
Example (accepted, coastal):
{{"rejected":false,"rejection_reason":null,"required_agents":["weather","ocean","tide"],"activity":"sea safety","date":"{today}"}}

Example (rejected, non-coastal coordinates):
{{"rejected":true,"rejection_reason":"{non_coastal_reason}","required_agents":[],"activity":"unknown","date":"{today}"}}
...
def planner_node(state):
    ...
    # --------------------------------------------------------
    # Deterministic coastal-proximity gate (runs before the LLM)
    # --------------------------------------------------------
    try:
        coastal_check = check_coastal_proximity.invoke(
            {
                "latitude": latitude,
                "longitude": longitude,
                "max_radius_km": COASTAL_CHECK_MAX_RADIUS_KM,
            }
        )
    except Exception as exc:
        # If the coastal check itself fails (e.g. dependency missing),
        # don't block the pipeline on it -- fall through and let the
        # LLM's own coastal judgement in the prompt be the only gate.
    ...
    if coastal_check is not None and not coastal_check.get("is_coastal", True):
        return {
            "plan": {
                "rejected": True,
                "rejection_reason": NON_COASTAL_REJECTION_REASON,
                "required_agents": [],
                "activity": "unknown",
                "date": _date.today().isoformat(),
                "grid_points": [],
    ...
    try:
        prompt_text = PLANNER_PROMPT.format(
            ...
            non_coastal_reason=NON_COASTAL_REJECTION_REASON,
        )
